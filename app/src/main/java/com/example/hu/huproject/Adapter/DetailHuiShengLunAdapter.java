...
public class DetailHuiShengLunAdapter extends BaseAdapter {
    private Context context;
    private List<HuiShengLunEntity> list;
    private Detail_HuiShengLun_fragment fragment;
    private List<Boolean> isClick = new ArrayList<>();//控件是否被点击,默认为false，如果被点击，改变值，控件根据值改变自身颜色


    public DetailHuiShengLunAdapter(Context context, List<HuiShengLunEntity> list, Detail_HuiShengLun_fragment fragment) {
    ...
    public View getView(final int i, View view, ViewGroup viewGroup) {
        ...
        viewHolder.button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                //删除数据库中的数据
                MyApp.getInstance().getmDaoSession().getHuiShengLunEntityDao().delete(list.get(i));
                list.remove(i);
