...
//数据管理中满载向下制动距离测试界面
public class Detail_KongZaiUp_fragment extends Fragment {
    ...
    TextView tvZhidongshijian;
    Unbinder unbinder;

    private KongZaiUpEntity kongZaiUpEntity1;
    private View view;
    ...
    public static Detail_KongZaiUp_fragment newInstance(Long id) {
        Detail_KongZaiUp_fragment fragment = new Detail_KongZaiUp_fragment();
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.fragment_detail_kongzaiup, container, false);
    ...
    public void initView() {
        if (view != null) {
            ...
            tvZhidongjiansudu = view.findViewById(R.id.tv_zhidongjiansudu);
            tvZhidongchusudu = view.findViewById(R.id.tv_zhidongchusudu);
            tvZhidongshijian = view.findViewById(R.id.tv_zhidongshijian);
    ...
    private void initChart() {
        ...
        kongZaiUpEntity1 = activity.kongZaiUpEntity;

        tvZhidongjiansudu.setText(kongZaiUpEntity1.getZhiDongJianSuDu());//制动减速度
        tvZhidongchusudu.setText(kongZaiUpEntity1.getZhiDongChuSuDu());//制动初速度
        tvZhidongshijian.setText(kongZaiUpEntity1.getZhiDongShiJian());//制动时间

        //不显示出来的数据不用作截取小数点处理：maxspeed
        renderer.setXAxisMin(0);
        double runtime = Double.parseDouble(kongZaiUpEntity1.getRunTimeTotal());
        renderer.setXAxisMax(runtime + 3);
        renderer.setYAxisMin(0);
        double maxspeed = Double.parseDouble(kongZaiUpEntity1.getMaxSpeed());
        renderer.setYAxisMax(maxspeed * 3 / 2);
        chart = ChartFactory.getCubeLineChartView(getActivity(), updateChart2(kongZaiUpEntity1), renderer, 0.1f);
    ...
    private XYMultipleSeriesDataset updateChart2(KongZaiUpEntity kongZaiUpEntity) {
        // 同样是需要数据dataset和视图渲染器renderer
        XYMultipleSeriesDataset mDataset = new XYMultipleSeriesDataset();
        // 设置好下一个需要增加的节点
        try {
            if (kongZaiUpEntity != null) {
                jsonArray_get = new JSONArray(kongZaiUpEntity.getPoint_speed());
            }
        ...
        String breakTime = kongZaiUpEntity.getBreakTime();
        String maxSpeed = kongZaiUpEntity.getMaxSpeed();
        String runTimeTotal = kongZaiUpEntity.getRunTimeTotal();
        //断点数据
