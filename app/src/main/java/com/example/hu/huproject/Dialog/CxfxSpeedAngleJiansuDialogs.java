...
public class CxfxSpeedAngleJiansuDialogs extends DialogFragment {
    ...
    private double maxSpeed_jianSudu;
    private double minSpeed_jianSudu;
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        speedAngleEntity = dbHelper.querySpeedAngleById(id);

        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        dialog.setContentView(R.layout.dialog_jiansu);
    ...
    private void initChartView() {
        //数据集合(运行时间)
        runTime = speedAngleEntity.getRunTimeTotal();
        max_speed = Double.parseDouble(speedAngleEntity.getMaxSpeed());
...
//                                view3.setLayoutParams(params);
                            ...
                            if (!etBegin.getText().toString().equals("") && !etEnd.getText().toString().equals("")) {
                                float v1 = Float.parseFloat(etBegin.getText().toString());//开始
                                float v2 = Float.parseFloat(etEnd.getText().toString());//结束
                                tvJiansuTime.setText(df1.format(v2 - v1));
                                int v12 = (int) (v1 * 10);
                                int v22 = (int) (v2 * 10);
                                //减速段的最大速度
                                maxSpeed_jianSudu = 0;
                                ...
                                try {
                                    if (v2 > v1) {
                                        //拿到保存的速度数组
                                        speed_jsonArray_get1 = new JSONArray(speedAngleEntity.getPoint_speed());
                                        if (speed_jsonArray_get1 != null && speed_jsonArray_get1.length() > 0) {
                                            for (int i = v12 + 1; i < speed_jsonArray_get1.length(); i++) {
                                                if (i > v22) {
                                                    break;
                                                }
                                                //得到单点速度
                                                ...
                                                //拿到最大速度和最小速度
                                                if (speed_linshi > maxSpeed_jianSudu) {
                                                    maxSpeed_jianSudu = speed_linshi;
                                                }
                                                if (speed_linshi < minSpeed_jianSudu) {
                                                    minSpeed_jianSudu = speed_linshi;
                                                }

                                            }
                                            chuSudu_jianSu = speed_jsonArray_get1.getDouble(v12 + 1);//初速度
                                            moSudu_jianSu = speed_jsonArray_get1.getDouble(v22); //末速度
                                            averageJianSudu = (chuSudu_jianSu - moSudu_jianSu) / jianSuTime;//平均减速度

                                            tvAverageJianSpeed.setText(df2.format(averageJianSudu));
                                            tvJiansuTime.setText(df1.format(jianSuTime));
                                            tvChuSudu.setText(df2.format(chuSudu_jianSu));
                                            tvMosudu.setText(df2.format(moSudu_jianSu));
                                            tvMaxSpeed.setText(df2.format(maxSpeed_jianSudu));
                                            tvMinSpeed.setText(df2.format(minSpeed_jianSudu));
                                        }
                                    }
        ...
        switch (view.getId()) {
            ...
            case R.id.btn_sure:
                ...
                if (begin.equals("") || end.equals("")) {
                    ...
                    speedAngleEntity.setJiansu_startTime(begin);
                    speedAngleEntity.setJiansu_endTime(end);
                    speedAngleEntity.setJiansu_avegare_jiansudu(df2.format(averageJianSudu));
                    speedAngleEntity.setJiansu_time(df2.format(jianSuTime));
                    ...
                    dbHelper.updateSpeedAngle(speedAngleEntity);
