...
//空载向上制动减速度测试的测试界面
public class KongZaiUpTestFragment extends ListFragment {
    ...
    @BindView(R.id.tv_zhidongjiansudu)
    TextView tvZhidongjiansudu;
    @BindView(R.id.ll1)
    LinearLayout ll1;
    ...
    private int endTimeCishu = 0;//划竖线次数，用此数来保证竖线只会划一次（结束时）
    ...
    private KongZaiUpEntity kongZaiUpEntity;
    private MyApp myApp;


    //实例化自己，可以从外部传参
    public static KongZaiUpTestFragment newInstance(int position) {
        KongZaiUpTestFragment testFragment = new KongZaiUpTestFragment();
    ...
    //计算制动减速度
    private String calculateZhiDongJianSuDu() {
        if (isGetBreakTime) {
            double zhiDongChuSuDu = (y_speed.get(s1_left - 1) + y_speed.get(s1_right - 1)) / 2;
            //制动初速度(取小数点后两位)
//            double d = (double) Math.round(zhiDongChuSuDu * 100) / 100;
            float csd = Float.parseFloat(df2.format(zhiDongChuSuDu));
            //制动时间
//            double zhiDongShiJian = (double) Math.round(addX1 - breakTime_double);
            float zdsj = Float.parseFloat(df2.format(addX1 - breakTime_double));
            return df2.format(csd / zdsj);
        } else {
            return "0";
        }
    }
    ...
    private void stop() {
        btnStart.setText("开始");
        isRunning = false;
        tvZhidongjiansudu.setText(calculateZhiDongJianSuDu());//制动减速度
                ...
                if (isTime1Connected && isSuduConnected) {
                    if (isRunning) {
                        ...
                        if (id_param == null) {
                            ...
                            kongZaiUpEntity = new KongZaiUpEntity();
                            kongZaiUpEntity.setKey(id_param);
                            mtaskEntity.setIs_KongZaiUpSave(true);
                            TaskEntityUtils.update(mtaskEntity);

                            kongZaiUpEntity.setCreateTime(tvTime.getText().toString());//创建时间
                            kongZaiUpEntity.setMaxSpeed(df2.format(max_speed));//最大速度(string类型)
                            kongZaiUpEntity.setRunTimeTotal(x_time);//运行时间(string类型),就是结束时间
                            jsonArray_set1 = new JSONArray();
                            for (double b : y_speed) {//y_speed是本次测试所有的速度集合
                                try {
                                    jsonArray_set1.put(b);
                            ...
                            kongZaiUpEntity.setPoint_speed(jsonArray_set1.toString()); //速度集合
                            kongZaiUpEntity.setBreakTime(df2.format(breakTime_double));//断开时间
                            kongZaiUpEntity.setZhiDongJianSuDu(tvZhidongjiansudu.getText().toString());//制动减速度
                            kongZaiUpEntity.setZhiDongChuSuDu(tvZhidongchusudu.getText().toString());//制动初速度
                            kongZaiUpEntity.setZhiDongShiJian(tvZhidongtime.getText().toString());//制动时间

                            //保存空载向上数据
                            myApp.getmDaoSession().getKongZaiUpEntityDao().insert(kongZaiUpEntity);
                            ...
                            new Handler().postDelayed(new Runnable() {
                                @Override
                                public void run() {
                                    ScreenShot.getPic(rl1, "空载向上制动减速度图_" + kongZaiUpEntity.getId());
                                }
                            }, 500);
                        }
                    }
    ...
    public class MySerialcontrol extends SerialHelper {
        ...
        protected void onDataReceived(final ComBean comBean) {
            ...
            if (comBean.recData[13] == -29 && isGetTime1Ack && isGetSuduAck) {
                getActivity().runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        tvZhidongjiansudu.setText("0");//制动减速度
            ...
            getActivity().runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    if (btnStart.getText().toString().trim().equals("结束")) {
    ...
    protected int setLayoutId() {
        return R.layout.fragment_kongzaiup_test;
    }
