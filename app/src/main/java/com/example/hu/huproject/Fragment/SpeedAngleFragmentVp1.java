...
//速度与角度==》》测试图
public class SpeedAngleFragmentVp1 extends Fragment {
    RelativeLayout relativeTitle;
    FrameLayout frameLayout1;
    FrameLayout frameLayout2;
    TextView speedYunxing;
    TextView angleYunxing;
    LinearLayout ll1;
    TextView speedMax;
    TextView angleMax;
    TextView tvTime;
    ...
    private List<Double> y_speed_Qj;//倾角点集集合
    private List<Double> y_current;
    private List<Double> y_speed_f;
    //运行时间（是一个string类型数据，表示多少秒）
    private String x_time;

    private JSONArray jsonArray_set1;//速度集合
    private JSONArray jsonArray_set2;//角度集合
    //    private JSONArray jsonArray_get;//查看结果时用来存取数据
    //速度与角度实体类
    private SpeedAngleEntity speedAngleEntity;
    private List<SpeedAngleEntity> speedAngleEntities;
    /**
     * 参数值
     */
    private double param_distance, param_current_transformation_ratio, param_max_current1, param_max_speed, param_max_current2;
    ...
    //现在角度和最大角度
    private double current_angle, max_angle;
    //最大电流（计算）
    //给定坐标存储
    ...
    private static final String TAG = "SpeedAngleFragmentVp1";
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        view = inflater.inflate(R.layout.fragment_speedanglevp1, container, false);
    ...
    public void initView() {
        if (view != null) {
            relativeTitle = view.findViewById(R.id.relative_title);
            ...
            angleYunxing = view.findViewById(R.id.angle_yunxing);
            ll1 = view.findViewById(R.id.ll1);
            speedMax = view.findViewById(R.id.speed_max);
            angleMax = view.findViewById(R.id.angle_max);
    ...
    public static SpeedAngleFragmentVp1 newInstance(int position) {
        SpeedAngleFragmentVp1 fragment = new SpeedAngleFragmentVp1();
    ...
    public void stop() {//停止画图（外部调用）
        if (mHandler_drawChart != null) {
            ...
            jsonArray_set2 = new JSONArray();
            for (double b : y_speed_Qj) {//y_speed是本次测试所有的速度集合
                try {
                    jsonArray_set2.put(b);
                } catch (JSONException e) {
                    e.printStackTrace();
                }
            }

            Log.i("speedanglefragmentvp1", "x_time== " + x_time + "\n" + "jsonArray长度为" + jsonArray_set1.length() + "\n" + "jsonArray为：" + jsonArray_set1.toString());
            //停止时保存数据
            MyApp.SpeedListX = x_time;//运行时间
            MyApp.jsonArray_SpeedListY = jsonArray_set1;//给速度集合赋值
//            MyApp.jsonArray_AngleListY = jsonArray_set2;//给角度集合赋值
    ...
    public void save1() {//保存，外部调用
        if (id_param == null || StringUtil.isEmpty(tvTime.getText().toString())
            ...
            speedAngleEntity = new SpeedAngleEntity();
            speedAngleEntity.setKey(id_param);
            mtaskEntity.setIs_SpeedAngleSave(true);
            TaskEntityUtils.update(mtaskEntity);
            speedAngleEntity.setTime(tvTime.getText().toString());//保存的是点击保存时的时间
            speedAngleEntity.setMaxSpeed(speedMax.getText().toString());//最大速度
            speedAngleEntity.setMaxAngle(angleMax.getText().toString());//最大角度
            speedAngleEntity.setRunTimeTotal(x_time);
            ...
            jsonArray_set2 = new JSONArray();
            for (double b : y_speed_Qj) {//y_speed_Qj是本次测试所有的角度集合
                try {
                    jsonArray_set2.put(b);
                } catch (JSONException e) {
                    e.printStackTrace();
            ...
            //保存速度集合
            speedAngleEntity.setPoint_speed(jsonArray_set1.toString());
            //保存角度集合
            speedAngleEntity.setPoint_angle(jsonArray_set2.toString());
            //保存速度与角度数据
            myApp.getmDaoSession().getSpeedAngleEntityDao().insert(speedAngleEntity);
            //保存任务的id，以便在匀速分析和减速分析界面拿到速度与角度实体类，能够更新
            MyApp.entityID = speedAngleEntity.getId();
            Log.i("vh", "id== " + speedAngleEntity.getId());
            Log.i("vh", "key== " + id_param);
            //截屏
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    ScreenShot.getPic(relativeTitle, "速度与角度图_" + speedAngleEntity.getId());
                    Log.i("kok", "截屏了");
                }
            }, 500);
        }
                ...
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_UP:
                        ...
                        if (seriesSelection != null) {
                            ...
                            } else {//seriesSelection当前点击点
                                if (seriesSelection.getSeriesIndex() == 1) {
                                    pv.setTextColor(Color.BLUE);
                                } else if (seriesSelection.getSeriesIndex() == 2) {
                                    pv.setTextColor(getResources().getColor(R.color.black));
                                } else if (seriesSelection.getSeriesIndex() == 4) {
                                    pv.setTextColor(getResources().getColor(R.color.color_text_press));
                                }
                                // 设置浮窗上显示的数值
                                pv.setValue(String.format("%.2f", seriesSelection.getValue()));
                            }
...
//    }
    ...
    private void setChartSettings(XYMultipleSeriesRenderer renderer) {
        ...
        renderer.setYAxisAlign(Paint.Align.RIGHT, 1);//Y轴所在位置
...
//        currentDataList = new ArrayList<>();
        ...
        y_speed_Qj = new ArrayList<Double>();
...
//                                int f = 0;
                                for (int i = 0; i < 10; i++) {
                                    ...
                                    //得到角度以及最大角度
                                    current_angle = MyApp.qjBean.yqj;
                                    //最大角度去绝对值
                                    if (Math.abs(current_angle) > max_angle) {
                                        max_angle = Math.abs(current_angle);
                                    }
                                    ...
                                    if (start_bl) {
                                        ...
                                        angleYunxing.setText(String.format("%.2f", current_angle));//运行角度
                                        angleMax.setText(String.format("%.2f", max_angle));//最大角度
                                        ...
                                        //角度的最大最小值设置为最大角度的 3/2
                                        renderer.setYAxisMin(-max_angle * 3 / 2, 1);
                                        renderer.setYAxisMax(max_angle * 3 / 2, 1);
//                                        renderer.setYAxisMax(100, 1);
                                        //干嘛用的？
//                                        f_count += addY1;
//                                        f += addY1;
                                        y_speed.add(addY1);
                                        y_speed_Qj.add(current_angle);
                                        mDataset.removeSeries(series1);
                                        mDataset.removeSeries(series2);
                                        series1.add(addX1, addY1);
                                        series2.add(addX1, current_angle);
                                        mDataset.addSeries(0, series1);
                                        mDataset.addSeries(1, series2);
