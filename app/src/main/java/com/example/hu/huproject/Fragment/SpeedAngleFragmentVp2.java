...
//速度与角度==》》匀速分析
public class SpeedAngleFragmentVp2 extends ListFragment {
    ...
    private double runTimeAverage;//用户选段的运行时间
    ...
    public static SpeedAngleFragmentVp2 newInstance(int position) {
        SpeedAngleFragmentVp2 fragment = new SpeedAngleFragmentVp2();
    ...
    protected void lazyLoadData() {
        ...
        yunSuDialogs.setOnClickListener(listener);
    ...
    private void initspeedData() {
        ...
        if (speedAngleEntity != null) {
            speedAngleEntity.setYunsu_startTime(startTime);//匀速运动起点
            speedAngleEntity.setYunsu_endTime(endTime);//匀速运动末点
            speedAngleEntity.setYunsu_runTime(runtime_str);//匀速运动时间
            speedAngleEntity.setYunsu_averageSpeed(averageSpeed);//匀速运动平均速度
            ...
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    ScreenShot.getPic(linearTitle, "速度与角度匀速分析图_" + speedAngleEntity.getId());
                }
            }, 500);
        }
    }
