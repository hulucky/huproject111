...
//速度==》》减速分析
public class SpeedFragmentVp3 extends ListFragment {
    ...
    public static SpeedFragmentVp3 newInstance(int position) {
        SpeedFragmentVp3 fragment = new SpeedFragmentVp3();
    ...
    private void initspeedData() {
        ...
        if (speedEntity != null) {
            speedEntity.setJiansu_startTime(startTime);//减速开始时间
            speedEntity.setJiansu_endTime(endTime);//减速结束时间
            speedEntity.setJiansu_avegare_jiansudu(df2.format(averageJianSudu));//平均减速度
            speedEntity.setJiansu_time(df2.format(jianSuTime));//减速时间
            ...
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    ScreenShot.getPic(linearTitle, "速度减速分析图_" + speedEntity.getId());
                }
            }, 500);
        }
    }
...
//                    totalSpeed += speed_addY1;
                ...
                chuSudu_jianSu = speed_jsonArray_get1.getDouble(startTimeDouble + 1);//初速度
                moSudu_jianSu = speed_jsonArray_get1.getDouble(endTimeDouble); //末速度
