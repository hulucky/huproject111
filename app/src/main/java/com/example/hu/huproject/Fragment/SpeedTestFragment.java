...
//速度测试的测试界面
public class SpeedTestFragment extends ListFragment {
    ...
    private SpeedFragmentVp1 vp1;//测试图
    private boolean isRunning = false;
    private boolean isConnected = false;//判断传感器是否连接
    private SpeedFragmentVp3 vp3;
    private SpeedFragmentVp2 vp2;
    private int startNum = 0;//开始按钮点击的次数
    private int saveNum = 0;//保存按钮点击的次数


    //实例化自己，可以从外部传参
    public static SpeedTestFragment newInstance(int position) {
        SpeedTestFragment testFragment = new SpeedTestFragment();
    ...
    protected void initView() {
        ...
        vp1 = SpeedFragmentVp1.newInstance(0);//测试图
        vp2 = SpeedFragmentVp2.newInstance(1);
        vp3 = SpeedFragmentVp3.newInstance(2);
        fragmentList.add(vp1);//测试图界面
                ...
                switch (sensorInf.getSensorType()) {
                    case 0://速度
                        getActivity().runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                isConnected = true;//收到数据时，传感器置为连接状态
                                ...
                                Log.i(MY_TAG, "电量: " + sensorInf.getPower() + "\n" + "信号强度：" + sensorInf.getSignal());
            ...
            case R.id.btn_save://保存
                if (isConnected) {
                    if (isRunning) {
                        ...
                        saveNum = saveNum + 1;
                        if (saveNum > startNum) {//这是为了在保存的时候重复数据只保存一次
                            break;
                        }
                        vp1.save1();
                        vp2.saveYunsu();
    ...
    public void onDestroy() {
        ...
        MyApp.entityID = null;//当退出速度测试界面时，把entityID置位null,保证下一次进来时，不会拿到上一次的任务
